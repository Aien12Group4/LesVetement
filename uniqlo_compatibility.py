...
def test_compatibility(args):
    ...
    with tf.Session() as sess:
        ...
        for outfit in dl.comp_outfits:
            ...
            # Cooper
            outputTXT.write("Set No:{} Mean scores between outfit: {:.4f}, label: {}\n".format(count, predicted_score, labelType))
            outputData['id'].append(str(count))
            outputData['score'].append(str(predicted_score))
            outputData['label'].append(str(labelType))

            print("[{}] Mean scores between outfit: {:.4f}, label: {}".format(count, predicted_score, labelType))
    ...
    # Cooper
    outputTXT.close()
    with open(outputJson, 'w') as obj:
        json.dump(outputData, obj)
